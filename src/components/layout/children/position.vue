<template>
        ...
        <el-table :data="dataList" border style="width: 100%">
            ...
            <el-table-column prop="address" label="操作">
                <template #default="{ row }">
                    <el-button type="primary" size="small" @click="change(row.id, row.positionName, row.describe)">修改</el-button>
                    <!-- <el-button type="danger" size="small">删除</el-button> -->
                    <el-popconfirm title="确认删除此部门吗？" confirm-button-text="确定" cancel-button-text="取消"
                        @confirm="del(row.id)">
                        <template #reference>
                            <el-button type="danger" size="small">删除</el-button>
                        </template>
    ...
    <!-- 点击添加角色弹出框 -->
    <!-- {{ tankuang }} -->
    <el-dialog v-model="open" :title="tankuang ? '添加职位' : '修改职位'" width="30%">
        <!-- 内容 -->

        <el-input v-model="inputrole" placeholder="请输入职位" />
...
import { Getposition, Addposition, Changeposition, Delposition } from '../../../servers/index'
...
const getpositiondata = () => {
    Getposition(params).then((res: any) => {
...
const opens = () => {
    ...
    let params: any = reactive({
        positionName: inputrole.value,
        describe: inputDescribe.value
    })
    // 请求接口
    Addposition(params).then((res: any) => {
        if (res.meta.code == 200) {
...
//修改接口
const changeopen = () => {
    let params: any = reactive({ id: partId.value, positionName: inputrole.value, describe: inputDescribe.value })
    console.log(params)
    console.log(partId.value)
    Changeposition(params).then((res: any) => {
        if (res.meta.code == 200) {
